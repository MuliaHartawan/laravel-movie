...
@section('content')
    <div class="movie-info border-b border-gray-800">
        <div class="container mx-auto px-4 py-16 flex flex-col md:flex-row">
            <div class="flex-none">
                <img src="{{ 'https://image.tmdb.org/t/p/w500/' . $actor['profile_path'] }}" alt="profile image" class="w-76">
            ...
            <div class="md:ml-24">
                ...
                <div class="flex flex-wrap items-center text-gray-400 text-sm">
                    ...
                    <span class="ml-2">{{ $actor['birthday'] }} ({{ \Carbon\Carbon::parse($actor['birthday'])->age }} years old) in {{ $actor['place_of_birth'] }}</span>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-8">
                    {{-- @foreach ($knownForMovies as $movie)
                        <div class="mt-4">
                            <a href="{{ $movie['linkToPage'] }}"><img src="{{ 'https://image.tmdb.org/t/p/w500/' . $movie['poster_path'] }}" alt="poster" class="hover:opacity-75 transition ease-in-out duration-150"></a>
    ...
    <div class="credits border-b border-gray-800">
        <div class="container mx-auto px-4 py-16">
            ...
            <ul class="list-disc leading-loose pl-5 mt-8">
                ...
                {{-- @dd($credit) --}}
                    <li>
                        @isset($credit['release_date'])
                            {{  \Carbon\Carbon::parse($credit['release_date'])->format('Y') }} &middot;
                        @endisset
                        @isset($credit['linkToPage'])
                            <strong><a href="{{ $credit['linkToPage'] ?  $credit['linkToPage'] : '#'}}" class="hover:underline">{{ $credit['title'] }}</a></strong>
                        @endisset
                         as 
                        @isset($credit['character'])
                            {{ $credit['character'] }}
                        @endisset
                     </li>
                @endforeach
